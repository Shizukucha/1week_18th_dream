...
public class SceneSwitcher : MonoBehaviour
{
	...
	void JUN_LastSceneRetry()
    {
		Debug.Log("リトライ");
		//最初のシーンへ遷移。
		//JUN_BGMManagerScript.instance.JUN_TransitionBGM(2);
		//JUN_SEManagerScript.instance.JUN_SettingPlaySE(1);

		//次のシーンへ遷移

		JUN_OnMaskToScene(RetryCheck);
	}

	void JUN_LastSceneTitle()
	{
		Debug.Log("タイトルへ");
		//最初のシーンへ遷移。
		//JUN_BGMManagerScript.instance.JUN_TransitionBGM(1);
		//JUN_SEManagerScript.instance.JUN_SettingPlaySE(0);
	...
	*/
	...
	public   void JUN_OnMaskToScene(int currentIndex)
    {
		...
		var mask = new ImageMaskTransition()
		{
			...
			// 2の時に0に行かない
			//nextScene = currentIndex

			//nextScene =1
			//nextScene = currentIndex == 1 ? 2 : 0
